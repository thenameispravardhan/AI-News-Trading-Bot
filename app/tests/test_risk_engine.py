...
# -- R14: Edge Memory gate (opt-in, default off) --------------------------


def _seed_losing_outcomes(db_session, symbol: str, action: str, n: int, move: float):
    ...
    for _ in range(n):
        db_session.add(
            SignalOutcome(
                ...
                price_at_signal=100.0, price_30m=100.0 * (1 + move / 100.0),
                move_30m_pct=move, move_5m_pct=move / 2.0, price_5m=100.0,
            )
        )
    db_session.commit()
...
@pytest.mark.asyncio
async def test_edge_gate_off_by_default_does_not_block(db_session, isolated_db):
    """A proven-loser track record must NOT block while the gate is off —
    but the edge is still computed into the decision context."""
    md = MarketDataBus()
    md.set_quote_sync("EDGX", last_price=100.0)
    strat = _make_strategy(db_session, "edge_off", config={"max_capital_risk_pct": 0.05})
    acct = _make_account(db_session)
    _seed_losing_outcomes(db_session, "EDGX", "BUY", 40, -2.0)  # clear loser
    sig = _make_signal(db_session, symbol="EDGX", action="BUY", strategy_id=strat.id)
    engine = RiskEngine(market_data=md, portfolio_value=50_000_000.0)
    decision = await engine.evaluate(
        signal=sig, strategy=strat, account=acct,
        entry=100.0, stop_loss=99.0, target=103.0, session=db_session,
    )
    assert decision.approved is True, decision.violations
    # Edge is surfaced regardless of the toggle.
    assert decision.context.get("edge") is not None
    assert decision.context["edge"]["expectancy"] == pytest.approx(-2.0)


@pytest.mark.asyncio
async def test_edge_gate_on_blocks_proven_loser(db_session, isolated_db):
    import os
    from app.config import get_settings

    md = MarketDataBus()
    md.set_quote_sync("EDGX", last_price=100.0)
    strat = _make_strategy(db_session, "edge_on", config={"max_capital_risk_pct": 0.05})
    acct = _make_account(db_session)
    _seed_losing_outcomes(db_session, "EDGX", "BUY", 40, -2.0)
    sig = _make_signal(db_session, symbol="EDGX", action="BUY", strategy_id=strat.id)
    engine = RiskEngine(market_data=md, portfolio_value=50_000_000.0)
    saved = os.environ.get("EDGE_GATE_ENABLED")
    try:
        os.environ["EDGE_GATE_ENABLED"] = "1"
        ...
        assert "RISK_EDGE_NEGATIVE" in codes
    finally:
        if saved is None:
            os.environ.pop("EDGE_GATE_ENABLED", None)
        else:
            os.environ["EDGE_GATE_ENABLED"] = saved
        get_settings.cache_clear()


@pytest.mark.asyncio
async def test_edge_gate_on_fails_open_on_thin_history(db_session, isolated_db):
    """Gate ON but only a few outcomes -> insufficient evidence -> allowed."""
    import os
    from app.config import get_settings

    md = MarketDataBus()
    md.set_quote_sync("THIN", last_price=100.0)
    strat = _make_strategy(db_session, "edge_thin", config={"max_capital_risk_pct": 0.05})
    acct = _make_account(db_session)
    _seed_losing_outcomes(db_session, "THIN", "BUY", 6, -3.0)  # < min_samples 30
    sig = _make_signal(db_session, symbol="THIN", action="BUY", strategy_id=strat.id)
    engine = RiskEngine(market_data=md, portfolio_value=50_000_000.0)
    saved = os.environ.get("EDGE_GATE_ENABLED")
    try:
        os.environ["EDGE_GATE_ENABLED"] = "1"
