  ...
  const onConnect = async () => {
    ...
    try {
      ...
      const watch = setInterval(() => {
        if (popup.closed) {
          clearInterval(watch);
          // The status poll will have already detected the token;
          // if not connected after a few seconds, surface an error.
          setTimeout(() => {
            if (!status?.connected) {
              // Stay in "waiting" a moment longer so the user
              // can see we're still checking, then reset.
              setMode("idle");
            }
          }, 1500);
        }
